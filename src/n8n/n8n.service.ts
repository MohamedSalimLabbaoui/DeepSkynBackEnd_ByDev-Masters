...
export class N8nService {
  ...
  async triggerReclamationWorkflow(payload: CreateReclamationDto) {
    ...
    for (const webhookUrl of webhookUrls) {
      try {
        const { data } = await axios.post(webhookUrl, webhookPayload, {
          headers: {
        ...
        if (axios.isAxiosError(error)) {
          ...
          if (this.configService.get<string>('N8N_RECLAMATION_WEBHOOK_URL')) {
            break;
          }
          continue;
        }
  ...
  async sendProcessedReclamationEmail(payload: ProcessReclamationDto) {
    ...
    await this.mailService.sendReclamationProcessedEmail(
      payload.email,
      userName,
    );
