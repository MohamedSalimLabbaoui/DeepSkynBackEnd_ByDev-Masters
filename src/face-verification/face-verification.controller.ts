...
export class FaceVerificationController {
  ...
  @ApiOperation({
    summary:
      "Vérifie si l'utilisateur a une référence faciale et une photo de profil",
  })
  @ApiResponse({
    status: 200,
  ...
  async getFaceStatus(@CurrentUser('userId') userId: string) {
    const hasFaceReference =
      await this.faceVerificationService.hasFaceReference(userId);
    const faceReference = hasFaceReference
      ? await this.faceVerificationService.getFaceReference(userId)
      : null;
    const profilePhotoUrl =
      await this.faceVerificationService.getProfilePhotoUrl(userId);
  ...
  @ApiOperation({
    summary:
      'Supprime la référence faciale pour permettre une réinitialisation',
  })
  @ApiResponse({
    status: 200,
  ...
  async resetFaceReference(@CurrentUser('userId') userId: string) {
    ...
    return {
      success: true,
      message:
        'Référence faciale supprimée. Veuillez enregistrer une nouvelle référence depuis votre photo de profil.',
    };
  }
}
