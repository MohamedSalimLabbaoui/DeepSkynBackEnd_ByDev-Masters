...
export class UsersService {
  ...
  async findSuggestions(userId: string, limit: number = 20) {
    ...
    const followingIds = connections.map((f) =>
      f.followerId === userId ? f.followingId : f.followerId,
    );
  ...
  async getUserStats(userId: string) {
    ...
    weeklyPosts.forEach((post) => {
      const diffDays = Math.floor(
        (now.getTime() - post.createdAt.getTime()) / (1000 * 3600 * 24),
      );
    ...
    const avatars = recentFollowers
      .map((f) =>
        f.followerId === userId ? f.following.avatar : f.follower.avatar,
      )
